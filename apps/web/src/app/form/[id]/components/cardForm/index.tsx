...
export const Question = ({
  ...
  const getOptions = async () => {
    const { data }: OptionsTypes = await api.get(
      `/api/question/option/show?question=${id}`,
    );
    setOptions(data);
  };
  ...
  const putOption = async (id: string | undefined) => {
    ...
    if (newOption.title !== '') {
      ...
      setOptions((state) =>
        state.map((option) =>
          option.id === id ? { ...option, title: 'Opção' } : option,
        ),
      );
  ...
  const changeOption = async (id: string | undefined, title: string) => {
    setOptions((state) =>
      state.map((option) =>
        option.id === id ? { ...option, title: title } : option,
      ),
    );
  };
      ...
      <div className={'w-3/4 border p-5 rounded-xl bg-gray-50'}>
        <div className={'space-y-3'}>
          <div className={'flex'}>
            <span
              className={
                'bg-white border rounded size-7 flex items-center justify-center mr-3 text-sm font-semibold'
              }
            >
              {index}
            </span>
