...
export const MenuOptionNewForm = ({
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 mt-2 flex items-center gap-4 bg-white w-min m-auto rounded-full p-1">
      <Button className="rounded-full" variant="ghost" size="icon" title={'Adicionar uma Questão do tipo texto'} onClick={() => createQuestion('TEXT')}>
        <StretchHorizontal size={20} />
      </Button>

      <Button className="rounded-full" variant="ghost" size="icon" title={'Adicionar uma Questão do tipo escolha'} onClick={() => createQuestion('CHOOSE')}>
        <CirclePlus size={20} />
      </Button>
    </div>
  )
