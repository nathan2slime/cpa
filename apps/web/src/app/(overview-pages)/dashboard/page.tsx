...
export default function Dashboard() {
  ...
  const allZeroValues = dashboardData.answersLastMonthAgrouped.every((item) => item.total === 0)
    ...
    <main className="w-full px-3 sm:px-4 md:px-5 py-4 md:py-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
        <Card className="overflow-hidden border-l-4 border-l-blue-500">
          ...
          <CardContent className="pb-4">
            <div className="flex flex-col">
              <span className="text-3xl sm:text-4xl font-bold text-blue-500">{dashboardData.openEvents}</span>
              <span className="text-xs sm:text-sm text-muted-foreground mt-1">Eventos aguardando conclusão</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden border-l-4 border-l-green-500">
          ...
          <CardContent className="pb-4">
            <div className="flex flex-col">
              <span className="text-3xl sm:text-4xl font-bold text-green-500">{dashboardData.actualMonthEvents}</span>
              <span className="text-xs sm:text-sm text-muted-foreground mt-1">Eventos registrados este mês</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden border-l-4 border-l-purple-500">
          ...
          <CardContent className="pb-4">
            <div className="flex flex-col">
              <span className="text-3xl sm:text-4xl font-bold text-purple-500">{dashboardData.actualMonthAnswers}</span>
              <span className="text-xs sm:text-sm text-muted-foreground mt-1">Interações realizadas este mês</span>
            </div>
          </CardContent>
        </Card>
      </div>
      <div className="mt-4 sm:mt-5 md:mt-6">
        <h2 className="text-base sm:text-lg font-semibold mb-2">Respostas do Último Mês</h2>
        <Card>
          <CardContent className="pt-6">
            {allZeroValues ? (
              <Alert className="mb-3 sm:mb-4 text-sm">
                <Info className="h-4 w-4" />
                <AlertTitle className="text-sm sm:text-base">Sem dados</AlertTitle>
            ...
            <div className="h-[250px] sm:h-[280px] md:h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
                  ...
                  <Bar dataKey="total" radius={[4, 4, 0, 0]} maxBarSize={50} animationDuration={1500}>
                    {chartData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.total > 0 ? "hsl(var(--primary))" : "hsl(var(--muted-foreground)/0.3)"}
                      />
                    ))}
                  </Bar>
                </BarChart>
