    ...
    <section className="bg-white w-max max-md:w-full rounded-lg min-h-96 p-4 grid grid-cols-1 gap-4 transition-all lg:grid-cols-2">
      <div className=" w-full border-r px-2 h-full">
        ...
        <div>
         ...
         items-center w-full"
          >
            <Select onValueChange={e => setCoursesSelected([...coursesSelected, e])}>
          ...
          <div className="mt-4 overflow-y-auto custom-scrollbar h-32 w-full bg-gray-100 rounded-lg text-sm">
            <ul className="">
              {coursesSelected.map((course, index) => (
                <li key={index} className="text-sm border-b p-1 flex justify-between items-center">
                  {course}
                  <Button className="w-4 h-4 -scale-75" variant="destructive" size="icon" onClick={() => setCoursesSelected(coursesSelected.filter(c => c !== course))}>
      ...
      <div className="py-2 w-full">
        <Select value={typeForm} onValueChange={e => setTypeForm(e)}>
          ...
          <SelectContent>
            <SelectItem value="avaliacao-institucional">Avaliação Institucional</SelectItem>
        ...
        <div className="mb-4 mt-4">
          ...
          <input type="text" className="w-full border-gray-300 rounded-md p-2" placeholder="Informe o responsável pelo evento" />
        </div>

        <div className="mb-4">
          <Label>Descrição:</Label>
          <textarea className="w-full border-gray-300 rounded-md p-2 resize-none overflow-y-auto" rows={4} placeholder="Descreva o evento" />
        </div>

        {typeForm === '' ? (
          <Button className="bg-green-300 hover:bg-green-400 hover:text-white transition-all delay-100" variant="secondary" disabled>
